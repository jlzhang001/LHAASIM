...
int
HitStationMD::TestThreshold (int trigtype)
{
  int flagtrig = 0;
  double threshold = 0;
  int bintrig = 0;
...
void
PMT_ED::FEFilter (map < Int_t, Double_t > &spec1, Double_t * spec2)
{
...
void
PMT_ED::FADCSampling (Double_t * spec1, Short_t * spec2)
{
  Int_t i, j, n;
  Double_t value;
  short firstbinfound = 0;
...
void
PMT_ED::FADCSamplingSat (Double_t * spec1, Short_t * spec2, Int_t * spec3)
{
...
HitStationED::ReadParticleInScint (ParticleInScint * p)
{
  ...
  for (Int_t np = 0; np < nph; np++) {
    Int_t ipm = p->fIpm;
    Int_t time = (Int_t) p->fTPhot;
    
    if (ipm > 0) {
      
      fPMT[ipm - 1].WritePE (p->fId, time );
      fPMT[0].WritePE (p->fId, time );
      //cout << "avant WritePE" << endl;
      WritePE (p->fId, time );
		      ...
		      const SampStation & sst)
{
  ...
  for (Int_t np = 0; np < npart; np++) {
    ...
    for (Int_t n = 0; n < nt; n++) {
      //  random entry point
      Double_t ri = (SCINT_LENGTH - eps) * sqrt (Rand ());
      Double_t azi = 2 * PI * Rand ();
      Double_t xx = ri * cos (azi);
      Double_t yy = ri * sin (azi);
      Double_t zz = SCINT_HEIGHT - eps;
      //   smoothed time (if multiple entries from one input particle)
      Double_t timr = timr0;
      if (nt + ns > 1)
	timr = timr0 * exp (sigt * Rand ("GAUSS", 0, 1));
      ...
      if(simmode != "SAMPLE") pclone->DoEDSim();
      if (simmode == "DETAILED") {
    ...
    for (Int_t n = 0; n < ns; n++) {
      //  random entry point
      Double_t yy = (SCINT_LENGTH - eps) * (2 * Rand () - 1.);
      Double_t xxx =
	sqrt ((SCINT_LENGTH - eps) * (SCINT_LENGTH - eps) - yy * yy);
      Double_t xx = (-xxx * cxpart + yy * cypart) / cxy;
      yy = (-xxx * cypart - yy * cxpart) / cxy;
      Double_t zz = (SCINT_HEIGHT - eps) * Rand ();
	...
	timr = timr0 * exp (sigt * Rand ("GAUSS", 0, 1));
      ...
      ParticleInScint *pclone =
	new ParticleInScint (fId, idpart, xx, yy, zz, cxpart, cypart, czpart,
			    timr, epart, wpart_det);
...
void
HitStationED::SetTime0 (double time0)
{
  fTime0 = time0;
}
...
int
HitStationED::TestThreshold (int trigtype)
{
  int flagtrig = 0;
  double threshold = 0;
  int bintrig = 0;
