...
extern Array *gArrayMD;
extern Array *gArrayED;
...
void
DoSampling (Double_t xcore, Double_t ycore,Int_t EDMD, vector < SampStation > *tank_list)
{
#ifndef CALIBONLY
  ...
  /*
  // Recensement des particules issues de la gerbe avant sampling et detection :
  TH1F *histo_Gamma = new TH1F("histo_Gamma","Gamma produced by proton shower function shower axis; Distance to shower axis (m);Number of gamma",10000,0.,10000.);
  TH1F *histo_Electron = new TH1F("histo_Electron","Electron produced by proton shower function shower axis; Distance to shower axis (m);Number of electron",10000,0.,10000.);
  TH1F *histo_Muon = new TH1F("histo_Muon","Muon produced by proton shower function shower axis; Distance to shower axis (m);Number of muon",10000,0.,10000.);
 
  //Déclaration des Canvas :
  TCanvas *cs = new TCanvas("cs","Particles shower",1);
  cs->Divide(3,1);
  */

  if(EDMD==1){
    area_top = PI * STATION_RADIUS * STATION_RADIUS;
    area_side = 2 * STATION_HEIGHT * STATION_RADIUS;
  } 
  else  if(EDMD==2 && theConfig()->EDMode == "Pb&Scint"){
    area_top =  PI * LEAD_RADIUS * LEAD_RADIUS;
    area_side = 2 * LEAD_HEIGHT * LEAD_RADIUS;
  } 
  else  if(EDMD==2 && theConfig()->EDMode == "Scint"){
    area_top =  PI * SCINT_RADIUS * SCINT_RADIUS;
    area_side = 2 * SCINT_HEIGHT * SCINT_RADIUS;
  } 
  ...
  if(EDMD==1)
    ...
    cout << "\tSampling in scintillators started " << endl;
  ...
  Station *st;
  if(EDMD==1)
    nombtank=gArrayMD->Size ();
  else if(EDMD==2)
    nombtank=gArrayED->Size ();
  
  for (Int_t ist = 0; ist < nombtank; ist++) {
   if(EDMD==1)
     st= &gArrayMD->fStationList[ist];
   else if(EDMD==2)
     st = &gArrayED->fStationList[ist];
   Double_t xstat = st->fEasting - xcore;
   Double_t ystat = st->fNorthing - ycore;
   Double_t zstat = st->fAltitude - ALTITUDE_OBSERVATORY;
  ...
  cout << "\tPreselected array of " << nombtanksel << " detectors" << endl;
  ...
  fflush (stdout);
  int pc = 0;
  // TROP DE PARTICULES, RACCOURCIS :
  //for (Int_t ip = 0; ip < 100000; ip++) {
    ...
    // if(abs(idpart) == 3)
    //   cout << "Dans ManageEvent :: " << x << " " << y << " " << cxpart << " " << cypart << " " << czpart << " " << thetapart << " " << epart << endl;

    // VIRE LES MUONS HYPER ENERGÉTIQUES (~8000-9000 GeV ->vérif. Corsika)
    if(abs(idpart) == 3 && epart > 500.) continue;
 ...
 /* ----------------------------------------------------------------------
       Trace les particules en fonction de leur distance à l'axe de la gerbe
       ---------------------------------------------------------------------*/
    
    // ouverture du fichier Part_shower.txt en écriture sans effacer contenu:
    /*
    ofstream fichier("/home/martraire/Bureau/Part_shower.txt", ios::out | ios::app); 	  
    if(fichier)
      {
	fichier << idpart << " " << rpart_sf << endl;
	fichier.close();
      }
    else
    ...
    */
    /*
    // Construction des histos :
    if (abs (idpart) == 1)
      histo_Gamma->Fill(rpart_sf,1);
    if (abs (idpart) == 2)
      histo_Electron->Fill(rpart_sf,1);
    if (abs (idpart) == 3)
      histo_Muon->Fill(rpart_sf,1);
    */
    /* --------------------------------------------------------------*/	    
	...
	if (abs (rstat_sf - rpart_sf) <= dr
	  ...
	  if (n_entry > 0) {
	    // if(abs(idpart) == 3) { 
	    //   cout << "Muon : " << wpart << " " << n_entry << " " << n_entry_top << " " << n_entry_side << " " << area_top << " " << area_side << " " << area_samp << " " << avrg_n <<  endl; 
	    //   cout << avrg_n << " = (" << wpart <<"*"<< area_side<< "*"<<-tantheta_part<<") / "<< area_samp<<")"<<endl;
	    // }
  ...
  // Écriture des histos et fichiers -> recensement des particules issues gerbe:
  /*
  cs->cd(1);
  histo_Gamma->Draw();
  cs->cd(2);  
  ...
  cs->SaveAs("/home/martraire/Bureau/Part.png");
  cs->SaveAs("/home/martraire/Bureau/Part.root");
  */  
...
HitStationMD *
GenerateHitStationMD (const SampStation & sst)
{
  ...
  HitStationMD *hst =
    new HitStationMD (sst.fId, sst.fNorthing, sst.fEasting, sst.fAltitude);
...
HitStationED *
GenerateHitStationED (const SampStation & sst)
{
  int nombpart = sst.fPartList.size ();
  ...
  HitStationED *hst =
    new HitStationED (sst.fId, sst.fNorthing, sst.fEasting, sst.fAltitude);

  if (nombpart <= 0) {
    return hst;
  }

  //cout << "\t Simulation of scintillator " << hst->fId << " ... ";
  hst->DoDetSim (theConfig ()->SimMode, costh_sh, sinth_sh, cosphi_sh,
		 sinphi_sh, sst);
  cout << endl;
  ...
  if (theConfig ()->SimMode == "DETAILED") {
    ...
    /*
    cout << "\t\tParticles in scintillator --- total number :" << hst->
      fNPartTot << endl;
    cout << "\t\t                  --- #muons       :" << hst->fNmu << endl;
    cout << "\t\t                  --- #photons     :" << hst->fNph << endl;
    cout << "\t\t                  --- #e+/e-       :" << hst->fNel << endl;
