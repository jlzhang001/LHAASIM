...
EventRec::EventRec(const Event & event):Event(event){
  fEnergyRec=0;
  fThetaRec=0;
  fAzimRec=0;
  fNorCoreRec=0;
...
//EventRec & EventRec::operator =(const EventRec & event){}

int EventRec::CountTrigMDStaVEM(float threshold){

  double chargeinvem;
  int n;
  n=0;
  for( int i=0;i<fHitStationListMD.size();i++){
    chargeinvem=fHitStationListMD[i].GetChargeInVEM();
    if(chargeinvem>threshold)n++;
  }
  return n;
    
}
int EventRec::CountTrigMDStaMu(int nmuth){
  int n,nmu;
  n=0;
  for( int i=0;i<fHitStationListMD.size();i++){
    nmu=fHitStationListMD[i].fNmu;
    if(nmu>nmuth)n++;
  }
  return n;
}  

int EventRec::CountTrigMDSta(int th){
  int n;
  n=0;
  for( int i=0;i<fHitStationListMD.size();i++){
    if(fHitStationListMD[i].TestThreshold(th))
       n++;
  }
  return n;
}  

int EventRec::CountTrigEDSta(int th){
  int n;
  n=0;
  for( int i=0;i<fHitStationListED.size();i++){
    if(fHitStationListED[i].TestThreshold(th))
       n++;
  }
  return n;
}  
