...
Station::Station (Int_t id, Float_t north, Float_t east)
{
  ...
  fNorthing = north;
  fEasting = east;
  fAltitude = ALTITUDE_OBSERVATORY;
  fX_sf = X_sf ();
  fY_sf = Y_sf ();
  fZ_sf = Z_sf ();
  fR_sf = R_sf ();
...
Station::X_sf ()
{
  ...
  Double_t zstat = fAltitude - ALTITUDE_OBSERVATORY;
...
void PMT_MD::WritePE (Int_t idpart, Int_t time)
{
...
void
PMT_MD::DirectLight (int npe_direct)
{
...
PMT_MD::DoElecSim ()
{
  ...
  if (theConfig ()->MuEmFlag) {
    ...
    fPMTSignal_hi_mu = pmtsignal_mu_hi; 
  }
}


void
PMT_MD::PMTAmplification (map < Int_t, Double_t > &spec, map < Int_t,
...
void
PMT_MD::FEFilter (map < Int_t, Double_t > &spec1, Double_t * spec2)
{
...
void
PMT_MD::FADCSampling (Double_t * spec1, Short_t * spec2)
{
  Int_t i, j, n;
  Double_t value;
...
void
PMT_MD::FADCSamplingSat (Double_t * spec1, Short_t * spec2, Int_t * spec3)
{
...
HitStationMD::HitStationMD (Int_t id, Float_t north, Float_t east, Float_t alt)
...
HitStationMD::HitStationMD (Int_t id, Float_t north, Float_t east)
...
void
HitStationMD::WriteSamplingFactor (Double_t sampfact)
{
  fSampFact = sampfact;
...
void
HitStationMD::WritePE (Int_t idpart, Int_t time)
{
...
void
HitStationMD::WriteADCBin (Int_t idpart, Int_t bin, Double_t bincontent)
{
  Short_t high;
  Short_t low;
...
void
HitStationMD::WriteDoubleADCBin (Int_t idpart, Int_t bin, Double_t bincontent)
{
...
void
HitStationMD::CountPart (Int_t idpart, Double_t wpart_det, Double_t time)
{
...
void
HitStationMD::WriteShortADCTraces ()
{
  Int_t npe = 0;
...
void
HitStationMD::ReadParticleInTank (ParticleInTank * p)
{
  Int_t nph = p->fNCherPhot;
  //cout << "Dans ReadInTank, nph = p->fNCherPhot = " << nph << endl;
  Int_t ndirect[NPM];
...
void
HitStationMD::DoDetSim (string simmode, double costheta_sh, double sintheta_sh,
		      ...
		      const SampStation & sst)
{
  ...
  for(Int_t np=0;np<npart;np++){
    ...
    //cout <<"timepart, time_plane: "<<timepart<<" "<<time_plane<<endl; //regarde timepart et plane
    ...
    if(timepart-time_plane<0)cout<<"error plan "<<timepart-time_plane<<" "<<time_plane<<endl;
  }
  if(mindiftimepart>1000000
     || (maxdiftimepart>1000000 && maxdiftimepart-mindiftimepart<1000000)){
  ...
  for (Int_t np = 0; np < npart; np++) {
    ...
    //cout<<mindiftimepart<<" "<<time_plane<<" "<<timr0<<endl;
	...
	timr = timr0 * exp (sigt * Rand ("GAUSS", 0, 1));
      ...
      /////////////////////////////////////////////////////////////////////////////////////////////////////
      // 1ÈRE APPROXIMATION : seul les muons passent dans la cuve !! Électrons et photons sont arrêtés !!
      /////////////////////////////////////////////////////////////////////////////////////////////////////
      //      cout << "dans Station.cc ->idpart = " << idpart << endl;
      
      if( abs(idpart) == 1 || abs(idpart) == 2 ) continue;
      // cout << "après continue si 1 ou 2 !! " << endl; 
      
      ParticleInTank *pclone = new ParticleInTank (fId, idpart, xx, yy, zz,
						   cxpart, cypart, czpart,
						   timr, epart, wpart_det);

      //cout << "--> ParticleInTank !! " << endl;

      this->CountPart (idpart, wpart_det / sampfact, timr);
      //cout<<"avant call top "<<n<<endl;
...
HitStationMD::SetTime0 ()
{
  ...
  fTime0 = SAMPLINGTIME * fTriggerBin - fZ_sf/CLIGHT;
}

void
HitStationMD::SetTime0 (double time0)
{
  fTime0 = time0;
}

void
HitStationMD::DoElecSim ()
{
...
int HitStationMD::TestThreshold (int th)
{
  int flagtrig = 0;
  int bintrig = 0;
  map<int,double> trace;
  
  trace = fTimeProfile_mu;
...
double HitStationMD::SignalInVem (){
  double signal = 0.;
  int satur = 0;
 ...
 double HitStationMD::GetChargeInVEM(){
   
   map<int,double> trace;
   double charge, chargeinvem;
...
void PMT_ED::WritePE (Int_t idpart, Int_t time)
{
  //cout << " Dans WritePE " << endl;
  if (fTimeProfile.count (time))
    fTimeProfile[time]++;
  else
    fTimeProfile[time] = 1;
...
void
PMT_ED::DirectLight (int npe_direct)
{
  fNpe_direct += npe_direct;
}
...
void
PMT_ED::PMTAmplification (map < Int_t, Double_t > &spec, map < Int_t,
		       Double_t > &specAn, map < Int_t, Double_t > &specDy)
{
...
void PMT_ED::FEFilter (map < Int_t, Double_t > &spec1, Double_t * spec2)
{
...
void PMT_ED::FADCSampling (Double_t * spec1, Short_t * spec2)
{
  Int_t i, j, n;
  Double_t value;
  short firstbinfound = 0;
...
void PMT_ED::FADCSamplingSat (Double_t * spec1, Short_t * spec2, Int_t * spec3)
{
...
HitStationED::HitStationED (Int_t id, Float_t north, Float_t east, Float_t alt)
  //  : SampStation(id, north, east, alt)
{
  fId = id;
  fNorthing = north;
...
HitStationED::HitStationED (Int_t id, Float_t north, Float_t east)
:SampStation (id, north, east)
{
  fId = id;
  fNorthing = north;
...
void
HitStationED::WriteSamplingFactor (Double_t sampfact)
{
  fSampFact = sampfact;
  for (Int_t ipm = 0; ipm < NPM; ipm++)
    fPMT[ipm].fSampFact = sampfact;
...
void
HitStationED::WritePE (Int_t idpart, Int_t time)
{
...
void
HitStationED::CountPart (Int_t idpart, Double_t wpart_det, Double_t time)
{
  fNPartTot += (Int_t) wpart_det;
  if (idpart == 1 || idpart == -1)
    fNph += (Int_t) wpart_det;
...
void
HitStationED::ReadParticleInScint (ParticleInScint * p)
{
  //cout << "Dans ReadParticleInScint" << endl;
  Int_t nph = p->fNPhot;
  //cout << "nph = fNPhot = " << nph << endl;
  Int_t ndirect[NPM];
  for (int ipm = 0; ipm < NPM; ipm++)
    ndirect[ipm] = 0;
  for (Int_t np = 0; np < nph; np++) {
    Int_t ipm = p->fIpm[np];
    Int_t time = (Int_t) p->fTPhot[np];
    //if(np < 50) cout << "### ipm = p->fIpm = " << ipm << ", time = p->fTPhot = " << time << ", fNPhot = " << nph <<" ####" << endl;
   
    if (ipm > 0) {   
      fPMT[ipm - 1].WritePE (p->fId, time);
      WritePE (p->fId, time);      
    }
  }
  int sumdirect = 0;
...
void
HitStationED::DoDetSim (string simmode, double costheta_sh, double sintheta_sh,
		      double cosphi_sh, double sinphi_sh,
		      const SampStation & sst)
{
  //cout << "Bienvenue dans DoDetSim de StationED.cc"<< endl;
  ...
  for (Int_t np = 0; np < npart; np++) {
    ...
    Double_t time_plane = sst.fPartList[np].fT_plane+mindiftimepart; //COBB
    //if(time_plane>timepart) 
    //cout << "attention" << timepart << " " << time_plane << endl;
    Double_t timr0 = max (timepart - time_plane, 0.);
    //cout << mindiftimepart << " " << time_plane << " " << timr0 << endl;
    Int_t n_entry_top = (Int_t) sst.fPartList[np].fWtop;
    Int_t n_entry_side = (Int_t) sst.fPartList[np].fWside;
    ...
    for (Int_t n = 0; n < nt; n++) {
      //  random entry point
      Double_t ri = (LEAD_RADIUS - eps) * sqrt (Rand ());
      Double_t azi = 2 * PI * Rand ();
      Double_t xx = ri * cos (azi);
      Double_t yy = ri * sin (azi);
      Double_t zz = LEAD_HEIGHT - eps;
      //   smoothed time (if multiple entries from one input particle)
      Double_t timr = timr0;
      if (nt + ns > 1)
	timr = timr0 * exp (sigt * Rand ("GAUSS", 0, 1));
      ...
      ParticleInScint *pclone = new ParticleInScint (fId, idpart, xx, yy, zz,
						   cxpart, cypart, czpart,
						   timr, epart, wpart_det);

      // cout << "--> ParticleInScint !! " << endl;
      ...
      if(simmode != "SAMPLE") 
	{
	  if(theConfig()->EDMode == "Pb&Scint") pclone->DoLeadSim();
	  if(theConfig()->EDMode == "Scint") pclone->DoScintSim();
	}
      //cout << "Before ReadParticleInScint : pclone->fNPhot = " << pclone->fNPhot << endl;
      
      if (simmode == "DETAILED") {
	ReadParticleInScint (pclone);
	//////cout << "une particule de plus" << endl;
	//here we count the pel for a summary
	Int_t nph = pclone->fNPhot;
	//cout << nph << endl;
	//cout << "After Read : pclone->fNPhot = " << nph << endl;
	Int_t nn = pclone->fNPhotList.size();

	for(int n=0; n < nn; n++)
	  cout << pclone->fNPhotList[n] << endl; 
	
	//cout << "StationList -> " << pclone->fNPhotList[n] << endl; 
	//////////////////
	// POUR STAT DR //
	//////////////////
	////cout << pclone->fNPhotList[n] << endl; 
	//cout << n << " " << pclone->fCodePhot[n] << "  --> " << pclone->fNPhotList[n] << endl; 
	//cout << "00000000" << endl;
	
	if (abs (idpart) == 1) //{
	  npel_ph += nph; // cout << "npel_ph += nph = " << npel_ph << endl;}
	else if (abs (idpart) == 2) //{
	  npel_el += nph;  //cout << "npel_el += nph = " << npel_el << endl; }
	else if (abs (idpart) == 3) //{
	  npel_mu += nph;  //cout << "npel_mu += nph = " << npel_mu << endl; }
    ...
    // side entries :
    // --> First approximation : particles cannot be detected by the side !

  }   	//end of loop on part in a tank
}


void HitStationED::SetTime0 ()
{
  int firstADCbin = MAXNUMBEROFTIMEBINS;
  int firstpm = 0;
...
void HitStationED::SetTime0 (double time0)
{
  fTime0 = time0;
}
...
int HitStationED::TestThreshold (int th)
{
  int flagtrig = 0;
  int bintrig = 0;
  map<int,double> trace;
...
// --- 
HitStationLightED::HitStationLightED()
{

}

HitStationLightED::HitStationLightED(const HitStationED& s)
{
  fTime0=s.fTime0;
  fNpe=s.fNpe;
  fNpe_mu=s.fNpe_mu;
  ...
  fNpe = fNpe; // * (QE_PMT * FiberPurcent);
  IsTriggered=((fNpe>=18)?1:0);
}
