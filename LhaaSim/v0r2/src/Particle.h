...
class ParticleInScint : public Particle
{
 ...
 public:
  ...
  /* step parameters for charged particles */
  Double_t ElectronEnergyLossLead;//!
  Double_t MuonEnergyLossLead;//!
  ...
  Double_t ProbNoInt;//!
  Double_t ProbPair;//!
  Double_t ProbIntLead;//!
  Double_t ProbPairLead;//!
  Double_t ProbComptLead;//!
  Double_t ProbPhotLead;//!

  Int_t fISta;
  Int_t fMotherId;  /* ID mother particle */
  Double_t fX;      /* X : position in m */
  Double_t fY;      /* Y : position in m */
  Double_t fZ;      /* Z : position in m */
  Double_t fW;	    /* W : weight */
  
  Int_t fNPhot;     /* Number of photons induced */
  //Double_t fTPhot;     /* Time of photons induced */
  //Int_t fIpm;       /* Number of PMT */
  Int_t WhereAmI;   /* 1->Pb, 2->Scint */
  Int_t AfterOneDR; /* 0->Pas passé dans DR, 1-> 1 DR */
  vector<Double_t> fBinContent;//!
  vector<Double_t> fTPhot;//!
  vector<Int_t> fNPhotList;//!
  vector<Int_t> fCodePhot;//!
  vector<Double_t> fIpm;//!
  ...
  void DoScintAfterDR();
  void GeomLengthLead();
  void GeomLengthScint();
  void InteractionProbabilityInLead();
  void InteractionProbabilityInScint();
  void DoGammaProcesses_Lead();
  void DoGammaProcesses_Scint();
  void ProbabilityProcess();
  void PairProbability();
  void PairProduction();
  void ComptonScattering();
  void PhotoElectricEffect();
  void PositionEnterScint(); 
  void PositionExitLead();
  bool EnergyLossInLead();
  void GenerateDeltaRays();
  bool EnergyLossInScint();
  void GeneratePhoton();
 
  
  ClassDef(ParticleInScint, 2)
