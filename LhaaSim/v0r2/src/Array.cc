...
Array::Array(string arrayfilename)
{
  ...
  cout << "stations coordinates in Easting Northing mode" << endl;
 
  while(!arrayfile.eof())
    {      
      arrayfile >> id >> east >> north >> altitude;
      nsta++;
      cout << "id: " << id << " -> " << east << " " <<  north << " " << altitude << endl;

      Station *sta = new Station(id,north,east,altitude);
      fStationList.push_back(*sta);
      delete sta;
  ...
  cout << " -> Nmin, Nmax , Emin , Emax = " << fNorMin << " " << fNorMax << " " << fEasMin << " " << fEasMax << endl;
}
